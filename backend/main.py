import json
import stellar # Importamos tu módulo de blockchain
...
def create_report(
    ...
    # --- INICIO INTEGRACIÓN BLOCKCHAIN (EVIDENCIA) ---
    tx_hash = stellar.registrar_evidencia_en_blockchain(
        stellar.SECRET_EMPRESA, 
        report_hash
    )
    # --- FIN INTEGRACIÓN ---
    ...
    report = models.Report(
        ...
        salt=salt,
        hash=report_hash,
        stellar_tx_hash=tx_hash, # Asignamos el hash de la transacción
        status="SUBMITTED",
    )
...
def update_report_status(
    ...
    # --- INICIO INTEGRACIÓN BLOCKCHAIN (PAGO Y REMEDIACIÓN) ---
    if new_status.value == "PAID":
        # Extraemos el monto numérico (ej. "500" de "500 TEST-XLM")
        monto_numerico = bounty.reward.split(" ")[0] 
        stellar.pagar_recompensa_desde_escrow(
            stellar.SECRET_EMPRESA, 
            stellar.PUBLIC_INVESTIGADOR, 
            monto_numerico
        )
    elif new_status.value == "REMEDIATED":
        stellar.registrar_proof_of_remediation(
            stellar.SECRET_EMPRESA, 
            report.hash
        )
    # --- FIN INTEGRACIÓN ---
